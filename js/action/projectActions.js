...
export const projectAction = {
  ...
  async loadProjects(initial = false) {
    ...
    try {
      ...
      if (!Array.isArray(projects)) {
        utils.showMessage("Limite alcanzado. Espere un minuto");
        return;
      }
          ...
          observer = new IntersectionObserver(
            (entries) => {
              if (entries[0].isIntersecting) {
                projectAction.loadProjects(false);
              }
            },
            {
              root: null,
  ...
  async loadMinimalProjects(sectionId, initial = false) {
      ...
      if (!miniObserver) {
        ...
        if (miniSentinel) {
          miniObserver = new IntersectionObserver(
            (entries) => {
              if (entries[0].isIntersecting) {
                projectAction.loadMinimalProjects("taskSection");
              }
            },
            {
              root: null,
  ...
  async createProjectAction(projectData, groupId) {
    try {
      ...
      if (
        response.detail === "Se ha creado un nuevo proyecto de forma exitosa"
      ) {
  ...
  async addUserToProjectAction(groupId, projectId, userId) {
    try {
  ...
  async refreshCurrentProject(groupId, projectId, title, description) {
  ...
  async editPermissionAction(groupId, projectId, userId, permission) {
    try {
  ...
  async removeUserFromProjectAction(groupId, projectId, userId) {
    try {
  ...
  async deleteProjectAction(groupId, projectId) {
    try {
  ...
  async editProjectAction(
    ...
    try {
      ...
      utils.showMessage("Proyecto editado exitosamente", "success");
      ...
      utils.showMessage("Error al editar el proyecto: ", error.message);
    }
  },
};
